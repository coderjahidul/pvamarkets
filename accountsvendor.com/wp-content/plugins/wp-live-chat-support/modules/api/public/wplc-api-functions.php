...
function wplc_check_user_request($cid) {
	...
	if(isset($_SESSION['wplc_session_chat_session_id'])){
		...
		if(intval($_SESSION['wplc_session_chat_session_id']) === $g_chat_id) {
			return true;
		}else {
			return false;
		}
...
function wplc_api_call_start_chat( WP_REST_Request $request ){
	...
	if ( !isset($_SESSION['wplc_session_chat_session_id']) && wplc_rate_limit_check() ) {
		$return_array = array();
...
function wplc_api_permission_check(){
	...
	if(isset($_SESSION['wplc_session_chat_session_id']) && check_ajax_referer( 'wp_rest', '_wpnonce', false )){
		return true;
	}else {	
    	return false;
	}
