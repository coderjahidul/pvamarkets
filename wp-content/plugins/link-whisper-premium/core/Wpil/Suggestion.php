...
//            $unique_keywords = self::getPostUniqueKeywords($target_keywords, $process_key);
        ...
        foreach ($phrases as $key_phrase => $phrase) {
            ...
            if(!empty($target_keywords)){
                ...
                foreach($target_keywords as $target_keyword){
                    ...
                    $in_stemmed = false !== strpos($stemmed_phrase, $target_keyword->stemmed) && !in_array($target_keyword->stemmed, $ignored_words, true); // if the keyword can be found after stemming it, and the stemmed version isn't an ignored word
                    $in_normalized = false;
                    $normalized = false;
                    ...
                    if(!$in_unstemmed && !$in_stemmed){
                        ...
                        $in_normalized = !empty($normalized) && false !== strpos($normalized, $target_keyword->normalized) && !in_array($target_keyword->normalized, $ignored_words, true); // if the keyword can be found after normalizing AND stemming it, and this version isn't an ignored word
                    }
    ...
    public static function getTitleWords($post, $target = null, $keyword = null, $count = null, $process_key = 0)
    {
        ...
        if(false && !empty($target) && $words = get_transient('wpil_inbound_title_words_' . $process_key)){// TODO: Remove at update 2.3.3 if no one complains about slower inbound Internal suggestions
            return $words;
        }else{
            ...
            // save the title words if this is an Inbound Search // TODO: Remove at update 2.3.3 if no one complains about slower inbound Internal suggestions
//            if(!empty($target)){
...
error_log(print_r($post, true));
    ...
    public static function getInboundSuggestedPosts($keyword, $excluded_posts)
    {
        ...
        if(!empty($same_language_posts) && $multi_lang){
            ...
            $posts = $wpdb->get_col("SELECT `ID` FROM {$wpdb->posts} WHERE post_type IN ('{$post_types}') $statuses_query AND ID NOT IN ({$excluded_posts}) {$age_query} {$selected_terms} {$post_content} ORDER BY ID DESC");
        }
