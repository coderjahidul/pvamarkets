...
class Wpil_Post
{
     ...
     */
    public static function addLinksToContent($data, $post, $unsanitized_postarr = array())
    {
        //get links from DB
...
//                $link['sentence'] = Wpil_Word::removeQuotes($link['sentence']);
            ...
            if (!empty($data_null)) {
                ...
                if(!Wpil_Base::has_hooked_function('save_post', 'Wpil_Post', 'addLinkToMetaContent', 9999)){
...
//            return;
        ...
        if (!empty($meta)) {
            ...
            if (!empty($fields)) {
                ...
                Wpil_Toolbox::update_encoded_post_meta($post_id, 'wpil_links', array_values($meta));
            }
        }
    }
    ...
    public static function insertLink(&$content, $sentence, $changed_sentence, $ignore_links = false, $offset = 0)
    {
        ...
        while($position_start !== false){
            ...
            if( !Wpil_Link::checkForForbiddenTags($examine_text, $replace, $sentence, $ignore_links) && 
                ...
                !self::check_if_inside_json($content, $position_start, $position_end))
            {
