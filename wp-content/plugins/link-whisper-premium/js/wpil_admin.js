		...
		jQuery.ajax({
			...
			success: function(response){
				...
				if(!response.completed){
					...
					return updateSuggestionDisplay(post_id, term_id, nonce, $el, 'inbound_suggestions', false, sameCategory, key, selectedCategory, sameTag, selectedTag, selectPostTypes, selectedPostTypes);
				}
			},
		...
		jQuery.ajax({
			...
			success: function(response){
				...
				if((count * response.batch_size) < response.post_count){
					...
					return updateSuggestionDisplay(post_id, term_id, nonce, $el, 'outbound_suggestions', linkOrphaned, sameCategory, key, selectedCategory, sameTag, selectedTag, selectPostTypes, selectedPostTypes);
				}else{
...
//				setupProcessingError(true);
	...
	function updateSuggestionDisplay(postId, termId, nonce, $el, type = 'outbound_suggestions', linkOrphaned, sameCategory = '', key = null, selectedCategory, sameTag, selectedTag, selectPostTypes, selectedPostTypes){
		jQuery.ajax({
			type: 'POST',
			url: ajaxurl,
			data: {
	...
	$(document).on('click', '#wpil-regenerate-suggestions', function(){
		if (!same_category_loading) {
			...
			url = url.replace(new RegExp("(&link_orphaned[^&]*)|(&same_category[^&]*)|(&same_tag[^&]*)|(&select_post_types[^&]*)|(&selected_category[^&]*)|(&selected_tag[^&]*)|(&selected_post_types[^&]*)", 'ig'), '');
	...
	$(document).on('change', '#field_link_orphaned, #field_same_category, #field_same_tag, #field_select_post_types, select[name="wpil_selected_category"], select[name="wpil_selected_tag"], select[name="selected_post_types"], .wpil-suggestions-can-be-regenerated', function(){
...
/*
		$.post(ajaxurl, { // TODO: DELETE IN 2.2.8 if no one misses it // The functionality is part of the system that forces a reload of AN existing report page when the "back to report" thing is called. That way, the link stats are updated when a "back to report" button is clicked. The problem is, that it needs to ping the server to see if the report needs reloading, and for some users, this is hitting limits. We'll see if this is a feature that's used widely, and delete it if there's no response to it's elimination.
