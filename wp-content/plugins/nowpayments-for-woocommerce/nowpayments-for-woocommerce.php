...
add_shortcode('crypto_callback', function(){
    $value = get_option("crypto_number");
    $value = !empty($value) ? $value + 1 : 1;

    update_option("crypto_number", $value);

    return $value;
});

add_shortcode('crypto_number', function(){
    $value = get_option("crypto_number");
