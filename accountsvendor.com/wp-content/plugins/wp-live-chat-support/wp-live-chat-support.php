...
function wplc_set_defaults() {
  ...
  if (empty($wplc_default_settings_array)) {
    $wplc_default_settings_array = array(
      ...
      'wplc_pro_na' => __("Chat offline. Leave a message", 'wp-live-chat-support'),
      'wplc_pro_offline1' => __("We are currently offline. Please leave a message and we'll get back to you shortly.", 'wp-live-chat-support'),
...
function wplc_filter_control_live_chat_box_html_ask_user_detail($wplc_settings) {
  ...
  if ($wplc_settings['wplc_loggedin_user_info'] && is_user_logged_in()) {
      ...
      if ($current_user->data != null) {
          ...
          $wplc_loggedin_user_name = esc_attr($current_user->user_nicename);
...
function wplc_filter_control_live_chat_box_above_main_div( $msg, $wplc_settings, $cid ) {
  ...
  '.$extra.'
  <button class="tcx_pullup down" title="'.esc_attr($title).'"><i class="fa fa-angle-down" ></i></button>
  </p>';
        }
      }
    }
	            ...
	            if(isset($_GET['wplc_confirm'])){
	                ...
	                if ($wplc_was_error) {
	                    echo "<div class='update-nag' style='margin-top: 0px;margin-bottom: 5px;'>
	                        ".__("Error: Could not delete chat", 'wp-live-chat-support')."<br>
	                      </div>";
	                } else {
	                     echo "<div class='update-nag' style='margin-top: 0px;margin-bottom: 5px;border-color:#67d552;'>
	                        ".__("Chat Deleted", 'wp-live-chat-support')."<br>
	                      </div>";
	                }
	                ...
	                echo "<div class='update-nag' style='margin-top: 0px;margin-bottom: 5px;'>
	                        ".__("Are you sure you would like to delete this chat?", 'wp-live-chat-support')."<br>
	                        <a class='button' href='?page=wplivechat-menu-history&wplc_action=remove_cid&cid=".esc_attr( $_GET['cid'] )."&wplc_confirm=1&wplc_history_nonce=" . $hist_nonce . "' >".__("Yes", 'wp-live-chat-support')."</a> <a class='button' href='?page=wplivechat-menu-history'>".__("No", 'wp-live-chat-support')."</a>
	                      </div>";
	            }
	        }
	    }
	  ...
	  <tr>
	    ...
	    if (!$results) {
	        ...
	        foreach ($results as $result) {
	            ...
	            $actions = "
	                <a href='$url' class='button' title='".__('View Chat History', 'wp-live-chat-support')."' target='_BLANK' id=''><i class='fa fa-eye'></i></a> <a href='$url2' class='button' title='".__('Download Chat History', 'wp-live-chat-support')."' target='_BLANK' id=''><i class='fa fa-download'></i></a> <a href='$url3' class='button'><i class='fa fa-trash-o'></i></a>      
	                ";
...
function wplc_filter_control_chat_header_extra_attr($wplc_extra_attr) {
    $wplc_settings = wplc_get_options();
    if ($wplc_settings['wplc_auto_pop_up']) { $wplc_extra_attr .= " wplc-auto-pop-up=\"".intval($wplc_settings['wplc_auto_pop_up'])."\""; }
...
<?php
    ...
    if (isset($_POST['wplc_exclude_post_types']) && ! empty($_POST['wplc_exclude_post_types'])) {foreach ( $_POST['wplc_exclude_post_types'] as $post_type ) { $wplc_data['wplc_exclude_post_types'][] = sanitize_text_field($post_type); } }
    ...
    $wplc_data['wplc_auto_pop_up'] = wplc_force_bool($_POST, 'wplc_auto_pop_up');
    ...
    $wplc_data['wplc_pro_chat_email_address'] = stripslashes(sanitize_text_field(wplc_force_string($_POST, 'wplc_pro_chat_email_address', $wplc_settings)));
    $wplc_data['wplc_social_fb'] = esc_url(wplc_force_url($_POST, 'wplc_social_fb', $wplc_settings));
    $wplc_data['wplc_social_tw'] = esc_url(wplc_force_url($_POST, 'wplc_social_tw', $wplc_settings));
...
<div class="wrap wplc_wrap">
    ...
    <div class="wplc_row_support">
        <div class='wplc_row_col_support' style='background-color:#FFF;'>
			<div class="wplc_panel_heading"><i class="fa fa-check-circle-o"></i> <?php _e("Plugin Features",'wp-live-chat-support'); ?></div>
        ...
        <div class='wplc_row_col_support' style='background-color:#FFF;'>
			<div class="wplc_panel_heading"><i class="fa fa-sliders"></i> <?php _e("Settings & Customization",'wp-live-chat-support'); ?></div>
			<div class="wplc_row_col_support_inner">
...
function wplc_transcript_add_admin_button( $cid ) {
  $wplc_settings = wplc_get_options();
	if ($wplc_settings['wplc_enable_transcripts']) {
		echo "<a href=\"javascript:void(0);\" cid='" . sanitize_text_field( $cid ) . "' class=\"wplc_admin_email_transcript button button-secondary\" id=\"wplc_admin_email_transcript\">" . __( "Email transcript to user", 'wp-live-chat-support') . "</a>";
	}
}
